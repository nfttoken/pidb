...
const TRANSLATE_BASE = "https://api-free.deepl.com/v2/translate";
...
const TRANSLATION_ENV_PATH = "D:\\codex\\.codex\\.env";
const TIMEOUT_MS = 15000;
const CHUNK_LIMIT = 450;

if (!PASSWORD) throw new Error("PIDB_ADMIN_PASSWORD is required");

function readTranslationSecrets(value) {
  const secrets = {};
  for (const line of String(value).split(/\r?\n/)) {
    const match = line.match(/^\s*(DEEPL_API_KEY|GOOGLE_TRANSLATE_KEY)\s*=\s*(.*?)\s*$/);
    if (!match) continue;
    ...
    secrets[match[1]] = secret;
  }
  return secrets;
}

const translationSecrets = readTranslationSecrets(await readFile(TRANSLATION_ENV_PATH, "utf8"));
const DEEPL_API_KEY = translationSecrets.DEEPL_API_KEY;
const GOOGLE_TRANSLATE_KEY = translationSecrets.GOOGLE_TRANSLATE_KEY;
if (!DEEPL_API_KEY || !GOOGLE_TRANSLATE_KEY) throw new Error("DEEPL_API_KEY and GOOGLE_TRANSLATE_KEY are required in D:\\codex\\.codex\\.env");
...
async function translateChunk(text) {
  ...
  const promise = (async () => {
    const query = text;
    try {
      const data = await request(TRANSLATE_BASE, {
        method: "POST",
      ...
      return { text: translated, provider: "DeepL API" };
    } catch (deeplError) {
      const url = new URL(GOOGLE_TRANSLATE_BASE);
      url.searchParams.set("key", GOOGLE_TRANSLATE_KEY);
      const data = await request(url, {
        method: "POST",
        headers: { accept: "application/json", "content-type": "application/json" },
        body: JSON.stringify({ q: query, source: "en", target: "zh-CN", format: "text" }),
      }, "Google Cloud Translation API");
      const translated = data?.data?.translations?.[0]?.translatedText || "";
      if (!translated) throw new Error(`DeepL and Google returned empty text: ${deeplError.message}`);
      translationRequests += 1;
      await sleep(1200 + Math.floor(Math.random() * 601));
      return { text: translated, provider: "Google Cloud Translation API" };
    }
  })();
...
async function translateProduct(product) {
  ...
  for (const claim of product.claims) translatedClaims.push(claim.claim_text_zh || await translate(claim.claim_text_en));
  return {
    description_zh: product.description_zh || await translate(product.description_en),
    how_to_use_zh: product.how_to_use_zh || await translate(product.how_to_use_en),
    warnings_zh: product.warnings_zh || await translate(product.warnings_en),
  ...
  for (const product of sheetMasks) {
    try {
      ...
      const attributes = {
        ...
        zh_translation_provider: translated.providers.length === 1 ? translated.providers[0] : translated.providers.join(" + ") || product.attributes?.zh_translation_provider || "DeepL API",
      ...
      if (results.length % 5 === 0 && results.length < sheetMasks.length) await sleep(3000 + Math.floor(Math.random() * 5000));
    } catch (error) {
