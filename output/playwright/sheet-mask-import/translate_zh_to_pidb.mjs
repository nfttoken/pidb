...
async function request(url, options = {}, label = url) {
  ...
  for (let attempt = 1; attempt <= 4; attempt += 1) {
    ...
    await sleep(retryAfter > 0 ? Math.min(retryAfter * 1000, 120000) : Math.min(15000, attempt * 2000));
  }
  throw lastError;
}
...
async function translateChunk(text) {
  ...
  const promise = (async () => {
    ...
    const data = await request(url, { headers: { accept: "application/json" } }, "Google translation");
    if (data?.responseStatus && data.responseStatus !== 200) throw new Error(`MyMemory translation status ${data.responseStatus}: ${data.responseDetails || "unknown error"}`);
    const translated = Array.isArray(data?.[0])
      ? data[0].filter(Array.isArray).map((segment) => segment[0] || "").join("")
      : "";
...
async function main() {
  ...
  const list = (await pidb("/products?offset=0&limit=100")).data.items;
  const details = await mapLimit(list, 4, async (item) => (await pidb(`/products/${item.id}`)).data);
  const sheetMasks = details.filter((item) => item.product_type.code === "sheet_mask" && item.attributes?.source_site === "Kiokii");
  const results = [];
  for (const product of sheetMasks) {
    try {
      ...
      const attributes = {
        ...
        zh_translation_provider: "Google Translate (gtx)",
