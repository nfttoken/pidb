...
async function main() {
  ...
  const [brands, productTypes, skinTypes, skinConcerns, ingredients, productList] = await Promise.all([
    data("/brands"), data("/product-types"), data("/skin-types"), data("/skin-concerns"), data("/ingredients"), data("/products?offset=0&limit=100"),
  ]);
  const type = productTypes.find((item) => /sheet\s*mask/i.test(item.name_en) || /sheet[_-]?mask/i.test(item.code));
  if (!type) throw new Error("Sheet Mask product type is missing from PIDB");
  ...
  for (const product of source.products) {
    ...
    const productCode = current?.product_code || `MASK-${asciiToken(vendor).slice(0, 18)}-${codeHash}`.slice(0, 50);
    const why = sectionText(product, "Why You'll Love It");
    ...
    const attributes = {
      ...(current?.attributes || {}), source_url: product.url, canonical_url: product.canonical_url || product.url,
  ...
  const report = {
    ...
    processed: results.length, created: results.filter((item) => item.created).length,
  ...
  console.log(JSON.stringify({ event: "complete", processed: report.processed, created: report.created, hosted_images: report.hosted_images, reused_hosted_images: report.reused_hosted_images, attached_images: report.attached_images, image_failures: report.image_failures.length, report: REPORT_PATH.pathname }));
}
