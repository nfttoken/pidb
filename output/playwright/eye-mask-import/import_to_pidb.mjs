...
const IMAGE_HOST = process.env.IMAGE_HOST || "https://img.web3er.eu.org";
...
function sourceProductType(product, productTypes, fallbackType) {
  const sourceType = norm(product.product_type);
  if (sourceType) {
    const sourceKeys = new Set([sourceType, sourceType.replace(/s$/, "")]);
    const matched = productTypes.find((item) => sourceKeys.has(norm(item.name_en)) || sourceKeys.has(norm(item.code)));
...
function offerPrice(product, variant) {
  const offers = Array.isArray(product.json_ld?.offers) ? product.json_ld.offers : [];
  const offer = offers.find((item) => variant.sku && String(item.sku || "") === String(variant.sku)) || offers[0];
  if (offer?.price != null && Number.isFinite(Number(offer.price))) return Number(offer.price);
  const raw = Number(variant.price);
  return Number.isFinite(raw) ? (raw >= 100 ? raw / 100 : raw) : null;
}
...
function buildSkus(product, vendor, usedSkus, usedBarcodes) {
  ...
  return variants.map((variant) => {
    ...
    return {
      ...
      suggested_price: offerPrice(product, variant),
      suggested_price_currency: "CAD",
      suggested_price_note: "Kiokii CAD price captured for review; verify against current supplier pricing.",
    };
  });
}
...
async function main() {
  ...
  const [brands, productTypes, skinTypes, skinConcerns, ingredients] = await Promise.all([
    data("/brands"), data("/product-types"), data("/skin-types"), data("/skin-concerns"), data("/ingredients"),
  ]);
  const productList = { items: [] };
  for (let offset = 0; ; offset += 100) {
    const page = await data(`/products?offset=${offset}&limit=100`);
    productList.items.push(...(page.items || []));
    if (productList.items.length >= (page.total || 0) || !(page.items || []).length) break;
  }
  let fallbackType = productTypes.find((item) => /eye\s*mask/i.test(item.name_en) || /eye[_-]?mask/i.test(item.code));
  if (!fallbackType) {
    fallbackType = (await pidb("/product-types", { method: "POST", body: JSON.stringify({ code: "eye_mask", name_en: "Eye Mask", name_zh: "眼膜", sort_order: 0 }) }, "create Eye Mask product type")).data;
    productTypes.push(fallbackType);
  }
  ...
  for (const item of existingDetails) {
    ...
    for (const key of ["source_url", "canonical_url"]) if (attrs[key]) existingSource.set(String(attrs[key]).replace("/collections/sheet-masks", "/products"), item);
  }
  ...
  for (const product of source.products) {
    ...
    const type = sourceProductType(product, productTypes, fallbackType);
    const stableUrl = (product.canonical_url || product.url).replace("/collections/sheet-masks", "/products");
  ...
  for (const row of productRows) {
    ...
    if (results.length % 5 === 0 && results.length < productRows.length) await sleep(4000 + Math.floor(Math.random() * 4001));
  }
