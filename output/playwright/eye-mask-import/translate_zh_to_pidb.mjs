...
const LOCAL_ENV_PATH = "D:/codex/.codex/.env";
const LOCAL_ENV = await readFile(LOCAL_ENV_PATH, "utf8").catch(() => "");
function localEnvValue(name) {
  const match = LOCAL_ENV.match(new RegExp(`^\\s*${name}\\s*=\\s*(?:["']([^"']+)["']|([^#\\r\\n]*?))\\s*$`, "m"));
  return match ? (match[1] || match[2] || "").trim() : "";
}
const DEEPL_API_KEY = localEnvValue("DEEPL_API_KEY");
const GOOGLE_TRANSLATE_KEY = localEnvValue("GOOGLE_TRANSLATE_KEY");
...
async function translateText(value, providers) {
...
async function translateProduct(product) {
  ...
  for (const claim of product.claims) translatedClaims.push(claim.claim_text_zh || await translateText(claim.claim_text_en, providers));
  return {
    description_zh: product.description_zh || await translateText(product.description_en, providers),
    how_to_use_zh: product.how_to_use_zh || await translateText(product.how_to_use_en, providers),
    warnings_zh: product.warnings_zh || await translateText(product.warnings_en, providers),
...
async function main() {
  ...
  const sheetMasks = await mapLimit(createdIds, 1, async (id) => (await pidb(`/products/${id}`)).data);
  const results = [];
  for (const product of sheetMasks) {
    try {
      const translated = await translateProduct(product);
      const { providers, ...translatedFields } = translated;
      const attributes = {
        ...(product.attributes || {}),
        zh_translation_status: "machine_translated_review_required",
        zh_translation_provider: providers.join(", ") || "Existing Chinese content",
      ...
      const payload = { ...translatedFields, attributes };
      ...
      if (results.length % 5 === 0 && results.length < sheetMasks.length) await sleep(4000 + Math.floor(Math.random() * 4001));
    } catch (error) {
